/////////////////////////////////////////////////////////////
//
// ZenMondo's Modeling Stand by ZenMondo Wormser
//
// Place this script in an object along with
// an assortment of animations.
//
// Sitting on the object will pose an avatar 1 meter
// above the object.
//
// Click the object to get a menu to select an animation
//
/////////////////////////////////////////////////////////////
...
DisplayMenu(key id)
{
    ...
    string b9 = (string) (menu_page *9 +9);
       ...
       list menu = [b1, b2, b3, b4, b5, b6, b7, b8, b9, "<<PREV", "Home", "NEXT>>"]; //will use the order_buttons function to put these in a good order for llDialog
...
default
{
     state_entry()
    {
        ...
        llSitTarget(<0.0, 0.0, 1.0>, ZERO_ROTATION);  //Position where Avatar will Sit
    ...
    {if(using) //Object in use
       {
            if(current_user != llDetectedKey(0))
            {
                ...
                llDialog(llDetectedKey(0), "I'm sorry another user is using me at this time, please wait a moment and try again.", ["OK"], 1181111811);
            }
    ...
    listen(integer channel, string name, key id, string message)
    {
        ...
        else //Patron Chose a Number
        {
            ...
            llStopAnimation("sit");
            llStartAnimation(llList2String(inventory_list, get_item));
            current_anim = llList2String(inventory_list, get_item);
            animating = TRUE;
